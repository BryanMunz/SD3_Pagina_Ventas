		...
		<div class="flex-w flex-sb-m p-b-52">
			...
			<style>
				...
				.flex-container>div {
					...
					/* Ancho ajustado para 6 columnas con márgenes */
					margin: 10px;
					text-align: center;
					line-height: 75px;
				}
				...
				<div class="wrap-filter flex-w bg6 w-full p-lr-40 p-t-27 p-lr-15-sm">
					<div class="">
						...
						if (count($data['categorias']) > 0) {
							...
							echo '<div class="flex-container">'; // Inicia la primera columna

							foreach ($categorias as $index => $categoria) {
								echo '<a href="' . base_url() . '/tienda/categoria/' . $categoria['idcategoria'] . '/' . $categoria['ruta'] . '" class="flex-c-m stext-107 cl6 size-301 bor7 p-lr-15 hov-tag1 trans-04 m-r-5 m-b-5">';
								echo $categoria['nombre'] . ' <span> &nbsp;(' . $categoria['cantidad'] . ')</span>';
								echo '</a>';

								if (($index + 1) % 6 == 0 && $index != count($categorias) - 1) {
									// Cierra la columna y abre una nueva después de cada conjunto de 6 elementos (excepto en el último conjunto)
									echo '</div><div class="flex-container">';
								}
							}

							echo '</div>'; // Cierra la última columna
						}
						?>
					</div>
				</div>
