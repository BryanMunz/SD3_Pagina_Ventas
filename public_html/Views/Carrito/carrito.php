...
?>
									...
									<tr class="table_row <?= $idProducto ?>">
										...
										<td class="column-4">
											<div class="wrap-num-product flex-w m-l-auto m-r-0">
												...
												<input class="mtext-104 cl3 txt-center num-product" type="number" name="num-product1" value="<?= $producto['cantidad'] ?>" idpr="<?= $idProducto ?>" onchange="updateCartItem(this)">
										...
										<td class="column-6">
											<div class="how-itemcart1" idpr="<?= $idProducto ?>" op="2" onclick="fntdelItem(this)" style="font-weight: bold; color: red; margin-right: 30px;">Eliminar</div>
										</td>
									</tr>
	...
	<script>
		function updateCartItem(input) {
			var idProducto = input.getAttribute('idpr');
			var cantidad = input.value;

			// Actualizar subtotal y total_iva
			var precio = <?= $producto['precio'] ?>;
			var totalProducto = precio * cantidad;
			var total_iva = totalProducto * <?= IVA ?>;
			var subtotal = totalProducto + total_iva;

			// Actualizar valores en la tabla
			document.querySelector('.<?= $idProducto ?> .column-5').innerText = '<?= SMONEY ?>' + formatMoney(totalProducto);
			document.getElementById('subTotalCompra').innerText = '<?= SMONEY ?>' + formatMoney(<?= $subtotal1 ?>);
			document.getElementById('totalCompra').innerText = '<?= SMONEY ?>' + formatMoney(<?= $subtotal + COSTOENVIO ?>);
		}
	</script>
<?php } else { ?>
	<br>
