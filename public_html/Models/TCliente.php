...
trait TCliente{
	...
	private $strconfimadocorreo;
	private $strEmail1;
	private $strcodigo1;
	private $idUsuario;
	
    public function verificarCuenta(int $idUsuarios, string $email, int $codigo, string $confimado) {
        $this->con = new Mysql();
        $this->idUsuario = $idUsuarios;
        $this->strcodigo1 = $codigo;
        $this->strEmail1 = $email;
        $this->strconfimadocorreo = $confimado;
    
        $sql = "SELECT * FROM persona WHERE codigo = '{$this->strcodigo1}'";
        $request = $this->con->select_all($sql);
    
        $sql = "UPDATE persona SET confirmado = ? WHERE idpersona = ? AND codigo = ?";
        $arrData = array($this->strconfimadocorreo, $this->idUsuario, $this->strcodigo1);
        $request_update = $this->con->update($sql, $arrData);
    
        if ($request_update) {
            // Consulta nuevamente la base de datos para obtener el código actualizado
            $sql = "SELECT codigo FROM persona WHERE idpersona = '{$this->idUsuario}'";
	...
	public function insertCliente(string $nombre, string $apellido, int $telefono, string $email, string $codigo, string $confimado, string $password, int $tipoid){
		$this->con = new Mysql();
		...
		if(empty($request))
		{
			$query_insert  = "INSERT INTO persona(nombres,apellidos,telefono,email_user,password,rolid,codigo,confirmado) 
							  VALUES(?,?,?,?,?,?,?,?)";
	...
	public function insertPedido(string $idtransaccionpaypal = NULL, string $datospaypal = NULL, int $personaid, float $costo_envio, float $costo_iva,  string $monto, string $monto_iva, int $tipopagoid, string $direccionenvio, string $status){
		$this->con = new Mysql();
		$query_insert  = "INSERT INTO pedido(idtransaccionpaypal,datospaypal,personaid,costo_envio,costo_iva,monto,monto_iva,tipopagoid,direccion_envio,status) 
							  VALUES(?,?,?,?,?,?,?,?,?,?)";
